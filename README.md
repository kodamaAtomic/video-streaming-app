...
#### **ビデオプレイヤー操作**
- `K` - 再生・一時停止
- `Space` - 2区間リピート設定（1回目：開始点、2回目：終了点）
- `C` - リピート区間リセット
- `←` `→` - 5秒スキップ（戻る・進む）
- `J` `L` - 10秒スキップ（戻る・進む）
- `M` - ミュート・ミュート解除
...
### 📊 **再生管理**
- **再生回数カウント** - 各動画の再生回数を自動追跡
- **再生回数表示** - サムネイル左上にオーバーレイ表示
- **フォルダ別永続化** - 登録フォルダごとに再生回数を個別保存
- **自動切り替え** - フォルダ変更時に自動的に対応する再生データを読み込み
- **ローカルストレージ保存** - ブラウザ終了後も再生回数を保持

### 🔍 **ソート・フィルタ機能**
- **3種類のソート** - 名前・再生回数・更新日時順
- **昇順・降順切り替え** - ボタンクリックで簡単切り替え
- **正規表現対応フィルタ** - 高度な検索機能
- **リアルタイム更新** - 入力と同時に表示が更新

### ⭐ **お気に入り機能**
- **動画のお気に入り登録** - サムネイル右上の星アイコンをクリック
- **フォルダ別管理** - 登録フォルダごとにお気に入りを個別保存
- **お気に入りタブ表示** - 登録した動画のみを一覧表示
- **永続化保存** - ブラウザ終了後もお気に入り状態を保持
...
#### **🔁 2区間リピート機能**
1. **開始点設定**: 動画再生中に`Space`キーを押す
2. **終了点設定**: 任意の位置で再度`Space`キーを押す
3. **自動ループ**: 設定区間が自動的にリピート再生される
4. **区間リセット**: `C`キーでリピート設定をクリア
5. **区間更新**: 新しい区間を設定すると既存区間は自動リセット
...
- **v1.7.0** - 再生回数・ソート・フィルタ機能実装
  - 動画再生回数の自動追跡・表示
  - 名前・再生回数・更新日時による3種類ソート機能
  - 昇順・降順の切り替え機能
  - 正規表現対応の高度なフィルタ機能
  - フォルダ変更まで再生データ保持
...
- **v1.6.0** - キーボードヘルプ機能実装
  ...
  - YouTube風のオーバーレイヘルプデザイン
  - プレイヤーモード/グリッドモード別の表示
  - 2区間リピート機能の詳細説明
